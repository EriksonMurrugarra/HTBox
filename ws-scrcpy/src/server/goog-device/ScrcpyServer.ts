...
export class ScrcpyServer {
    ...
    public static async run(device: Device): Promise<number[] | undefined> {
        ...
        runPromise
            .then((out) => {
                if (device.isConnected()) {
                    // Note: Clipboard errors (NoSuchMethodException) on Android 14+ are expected
                    // and non-fatal. The server should continue to work despite these errors.
                    if (out && out.includes('NoSuchMethodException') && out.includes('clipboard')) {
                        console.warn(
                            device.TAG,
                            'Clipboard error detected (expected on Android 14+). Server should continue to work.',
                        );
                    } else if (out) {
