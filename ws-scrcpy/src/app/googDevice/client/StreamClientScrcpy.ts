    ...
    implements KeyEventListener, InteractionHandlerListener
{
    ...
    public onVideo = (data: ArrayBuffer): void => {
        ...
        // Force play if paused - this fixes black screen issue
        if (currentState === STATE.PAUSED) {
            this.player.play();
        }
        
        // Push frame - most players can handle frames even when transitioning to PLAYING
        // This ensures frames are queued and will be displayed once player starts
        try {
            this.player.pushFrame(new Uint8Array(data));
        } catch (e) {
            // Some players may reject frames when not fully initialized
            // This is okay, frames will be handled when player is ready
        }
        
        // Auto-play timeout: if we're receiving frames but player is still paused,
        // force play after a short delay (fallback mechanism)
        if (this.player.getState() === STATE.PAUSED && !this.autoPlayTimeout) {
            this.autoPlayTimeout = window.setTimeout(() => {
                if (this.player && this.player.getState() === STATE.PAUSED) {
                    console.log(TAG, 'Auto-playing player after receiving frames');
                    this.player.play();
                }
    ...
    public onDisplayInfo = (infoArray: DisplayCombinedInfo[]): void => {
        ...
        // Force play when display info is received
        if (this.player.getState() === BasePlayer.STATE.PAUSED) {
            console.log(TAG, 'Playing player after receiving display info');
            this.player.play();
        }
        // Clear auto-play timeout if display info arrives
